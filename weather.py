...
def print_data(final_list, num):
    ...
    for i in range(num):
        ...
        msg_out = msg_out+("{:^10}\t{:^8}\t{:^8}\t{:^8}\t{:^8}".format(final[0], final[1], final[2], final[3], final[4]))+("\n")
    return msg_out

# 用main()主函数将模块连接
def main():
    url = 'http://www.weather.com.cn/weather/101210507.shtml'
    html = getHTMLText(url)
    final_list = get_data(html)
    return print_data(final_list,2)


def return_main():
    return_main_0 = main()
    return return_main_0